...
int stats_poll(int map_fd, __u32 map_type, __u32 map_id, int interval, char* pin_dir)
{
...
int main(int argc, char **argv)
{
    ...
    while (1) {
		...
		int err = stats_poll(stats_map_fd, info.type, info.id, interval, pin_dir);
    	close(stats_map_fd);
    	if (err < 0)
    		return err;
    }
